...
class LinearRegression:
    ...
    def predict_model(self, model, sample_number:int = 0):
        """Predict the model"""
        prediction = model.predict(self.X_test[sample_number].reshape(1, -1))
        return prediction , self.y_test[sample_number] 

    def plot_to_base64(self, fig):
        """Plot the model to base64"""
        buf = io.BytesIO()
    ...
    def plot_decision_boundary(self, model):
        ...
        axs.contourf(xx, yy, Z, alpha=0.3, cmap='viridis')
        return self.plot_to_base64(fig)
